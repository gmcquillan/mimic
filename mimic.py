...
class ExpectedMethodCallsError(Error):
    ...
    def __init__(self, expected_methods):
        """Init exception.
        Args:
            # expected_methods: A sequence of MockMethod objects that
        ...
        def __str__(self):
            ...
            calls = "\n".join([
                    "%3d.  %s" % (i, m) for i, m in enumerate(
                        self._expected_methods
                    )
            ])
...
class UnexpectedMethodCallError(Error):
    ...
    This can occur if a method is called with incorrect parameters, 
    or out of the specified order.
    ...
    def __init__(self, unexpected_method, expected):
        ...
        Args:
            # unexpected_method: MockMethod that was called but was 
            # not at the head of the expected_method queue.
        ...
        if expected is None:
            ...
            diff = differ.compare(
                str(unexpected_method).splitlines(True),
...
class UnknownMethodCallError(Error):
    ...
    def __init__(self, unknown_method_name):
        ...
        Args:
            # unknown_method_name: Method call that is not part of the
            # mocked class's public interface.
...
class PrivateAttributeError(Error):
    ...
    def __str__(self):
        return (
            "Attribute '%s' is private and should not be available in a mock "
            "object." % self._attr
        )
...
class UnexpectedMockCreationError(Error):
    ...
    def __str__(self):
        args = ", ".join([
            "%s" % v for i, v in enumerate(self._params)
        ])
        ...
        if self._named_params:
            error += ", " + ", ".join([
                "%s=%s" % (k, v) for k, v in self._named_params.iteritems()
            ])
...
class SwallowedExceptionError(Error):
    """Raised when Verify() is called after something already raised exception.
    ...
    def __str__(self):
        exceptions = "\n".join([
            "%3d.  %s: %s" % (i, e.__class__.__name__, e) for i, e in enumerate(
                self._previous_exceptions
            )
        ])
...
class Mimic(object):
    ...
    _USE_MOCK_OBJECT = [
        types.ClassType, types.FunctionType, types.InstanceType,
    ...
    def create_mock(self, class_to_mock, attrs=None):
        ...
        Args:
            ...
            class_to_mock: class
            attrs: dict of attribute names to values that will be set on 
                   the mock object.  Only public attributes may be set.
    ...
    def CreateMockAnything(self, *args, **kwargs):
        return self.create_mock_anything(*args, **kwargs)
    ...
    def StubOutWithMock(self, *args, **kwargs):
        return self.stub_out_with_mock(*args, **kwargs)
    ...
    def StubOutClassWithMocks(self, *args, **kwargs):
        return self.stub_out_class_with_mocks(*args, **kwargs)
    ...
    def Replay(*args, **kwargs):
        #TODO(gavin): make sure this is okay with reference to self.
        # Might want to make this a staticmethod
        # TODO(gavin) Stopped here
...
class MockAnything:
    ...
    def __eq__(self, rhs):
        ...
        return (
            isinstance(rhs, MockAnything) and
...
class MockObject(MockAnything, object):
    ...
    def __init__(self, class_to_mock, attrs=None):
        ...
        for attr, value in attrs.items():
            if attr.startswith("_"):
                ...
                raise ValueError("'%s' is a method of '%s' objects." % (
                    attr, class_to_mock
                ))
        else:
    ...
    def __setitem__(self, key, value):
        """Provide custom logic for mocking classes that support assignment.
        ...
        Returns:
            Expected return value in replay mode.
            A MockMethod object for the __setitem__ method that has 
            already been called if not in replay mode.
    ...
    def __getitem__(self, key):
        ...
        Returns:
            ...
            __getitem__ method that has already been called if not in
            replay mode.
    ...
    def __contains__(self, key):
        ...
        Returns:
            Expected return value in replay mode.  A MockMethod object for the
            __contains__ method that has already been called if not in replaying
            mode.
        ...
        if self._replay_mode:
            if not self._instance_queue:
                exception = UnexpectedMockCreationError(
                    self._class_to_mock, *args, **kwargs
                )
                self._exceptions_thrown.append(exception)
...
class MethodSignatureChecker(object):
    ...
    def __init__(self, method):
        ...
        Raises:
            ...
            possible. Some methods and functions like built-ins can't being
            inspected.
        ...
        try:
            ...
            raise ValueError(
                'Could not get argument specification for %r'
                % (method,)
            )
    ...
    def _RecordArgumentGiven(self, arg_name, arg_status):
        ...
        if arg_status.get(arg_name, None) == MethodSignatureChecker._GIVEN:
            raise AttributeError(
                '%s provided more than once' % (arg_name,)
            )
    ...
    def Check(self, args, kwargs):
        ...
        if inspect.ismethod(self._method):
            ...
            if len(args) > len(self._required_args):
                ...
                try:
                    param_equality = (args[0] == expected)
                except:
                    param_equality = False;

                if isinstance(args[0], expected) or param_equality:
                    args = args[1:]
                ...
                # For example, the code under test does
                # late binding to a subclass.
                elif (
                    isinstance(args[0], IsA) and
                    args[0]._IsSubClass(expected)
                ):
                    args = args[1:]
            ...
            still_needed = [
                k for k, v in arg_status.iteritems() 
                    if v == MethodSignatureChecker._NEEDED
            ]

            if still_needed:
                raise AttributeError(
                    'No values given for arguments: %s'
...
class MockMethod(object):
    ...
    def __init__(self, method_name, call_queue, exception_list, replay_mode,
        ...
        Args:
            ...
            # replay_mode: False if we are recording, True if we are 
            #              verifying call against the call queue.
            # method_to_mock: The actual method being mocked, used for
            #                 introspection.
            # description: optionally, a descriptive name for this method.
            #              Typically this is equal to the descriptive name 
    ...
    def __call__(self, *args, **kwargs):
        ...
        If the mock is in replay mode, this will pop a MockMethod off the top
        of the queue and verify this call is equal to the expected call.

        Raises:
            UnexpectedMethodCall if this call is supposed to match an 
                expected_method method call and it does not.
        ...
        if expected_method._side_effects:
            result = expected_method._side_effects(*args, **kwargs)
    ...
    def InAnyOrder(self, group_name="default"):
        ...
        There can be multiple groups that are expected serially, if they
        are given different group names.  The same group name can be reused
        if there is a standard method call, or a group with a different name,
        spliced between usages.
    ...
    def MultipleTimes(self, group_name="default"):
        """Move this method into group which may be called multiple times.

        A group of repeating calls must be defined together, and must before
        executed in full before the next expected method can be called.
...
class Not(Comparator):
    ...
    Example:
        mock_dao.UpdateUsers(Not(
            ContainsKeyValue('stevepm', stevepm_user_info)
        ))
    """
    ...
    def __init__(self, predicate):
        ...
        """
        assert (
            isinstance(predicate, Comparator),
...
class SameElementsAs(Comparator):
    ...
    def equals(self, actual_seq):
        ...
        """
        try:
            # Store in case actual_seq is an iterator.  We potentially 
            # iterate twice: once to make the dict, once in the list fallback.
            actual_list = list(actual_seq)
        except TypeError:
            # actual_seq cannot be read as a sequence.
            #
            # This happens because Mimic uses __eq__ both to check object 
            # equality (in MethodSignatureChecker) and to invoke Comparators.
            return False

        try:
            expected = dict(
                [(element, None) for element in self._expected_list]
            )
...
class And(Comparator):
    """Returns an AND of the results for rhs comparators."""
...
class Or(Comparator):
    """Returns an OR of the results for rhs comparators."""
    ...
    def MethodCalled(self, mock_method):
        ...
        for method in self._methods:
            if method == mock_method:
                ...
                # equality is checked during removal.  The method in the 
                # group could pass a comparator to
                # another comparator during the equality check.
                self._methods.remove(mock_method)

                # If this group is not empty, put it back at the head of there
                # queue.
...
class MimicMetaTestBase(type):
    ...
    def __init__(cls, name, bases, d):
        ...
        # for a case when test class is not the immediate child of
        # MimicTestBase
        for base in bases:
        ...
        for func_name, func in d.items():
            if func_name.startswith('test') and callable(func):
                setattr(
                    cls, func_name, MimicMetaTestBase.CleanUpTest(cls, func)
                )
    ...
    def CleanUpTest(cls, func):
        ...
        Args:
            cls: MimicTestBase or subclass; the class whose test method we
                 are altering.
            func: method; the method of the MimicTestBase test class we wish
                  to alter.
...
class MimicTestBase(unittest.TestCase):
    ...
    Sets up a "mimic" attribute which is an instance of Mimic (any mimic tests
    will want this), and a "stubs" attribute that is an instance of 
